...
def generate_alerts(traffic_data, deviations, threshold):
    ...
    for i in range(len(traffic_data)):
        if abs(deviations[i]) > threshold:
            alert = {
                'Timestamp': traffic_data['Timestamp'][i],
                'Traffic Value': traffic_data['Value'][i],
...
def output_alerts(alerts):
    for alert in alerts:
        print(f"Alert: Timestamp={alert['Timestamp']}, Traffic Value={alert['Traffic Value']}, Deviation={alert['Deviation']}")
